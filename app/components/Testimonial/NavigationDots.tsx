...
export const NavigationDots: React.FC<NavigationDotsProps> = ({
  ...
  return (
    ...
    >
      <button
        onClick={onPrevious}
        className="justify-center items-center bg-white flex min-h-10 flex-col overflow-hidden w-10 h-10 px-2.5 rounded-[100px] border-2 border-solid border-black hover:bg-gray-50 transition-colors"
      ...
      >
        <div className="flex min-h-5 w-5 items-center gap-2.5 justify-center">
          <img
            ...
            alt=""
            className="object-contain w-1 stroke-[2px] stroke-black"
            aria-hidden="true"
          />
        </div>
      </button>
      ...
      {Array.from({ length: totalDots }, (_, index) => (
        <button
          ...
          className={`flex shrink-0 h-2 rounded-[100px] transition-all hover:scale-110 ${
            index === currentIndex
              ? "items-center border gap-2.5 overflow-hidden w-4 h-4 p-1 border-solid border-secondary"
              : "w-2 bg-black"
          }`}
        ...
        >
          {index === currentIndex && (
            <div className="flex min-h-2 w-2 h-2 bg-secondary rounded-[100px]" />
          )}
        </button>
      ))}
